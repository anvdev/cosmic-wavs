...
{/* todo: quick technical review of this page. Add info on the registry???*/}
...
4. Each operator signs the result of their off-chain computation and submits it to an aggregator.

5. The aggregator accepts results from the operators and verifies that the result payloads match and that the signatures are valid. Once enough verified submissions are received (the threshold is defined in the service manager), the aggregator submits the bundled ECDSA signatures and result payloads as a single transaction to the submission contract specified in the service's manifest.
...
{/* todo: add info on the registry */}
The WAVS registry is a decentralized registry of service components...
...
To learn more about the aggregator, visit the [Submission and aggregator page](./handbook/submission#aggregator).
