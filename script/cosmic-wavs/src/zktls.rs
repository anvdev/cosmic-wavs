...
impl ClaimInfo {
    pub fn serialize_for_identifier(&self) -> String {
        let ClaimInfo { provider, parameters, context_address, context_message } = self;
...
impl ClaimData {
    pub fn serialize_for_recovery(&self) -> String {
        let ClaimData { identifier, owner, timestamp, epoch_index } = self;
    ...
    pub fn serialize_for_witness(&self, epoch_minimum_witness: usize) -> String {
        let ClaimData { identifier, timestamp, epoch_index, .. } = self;
