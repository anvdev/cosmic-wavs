...
pub const BITSONG_NETWORK: NetworkInfo =
    NetworkInfo { chain_name: "Bitsong", pub_address_prefix: "bitsong", coin_type: 639u32 };
...
// Localnet
const LOCAL_NET: NetworkInfo =
    NetworkInfo { chain_name: "Local Network", pub_address_prefix: "mock", coin_type: 114u32 };
...
pub async fn ping_grpc(url_str: &str) -> anyhow::Result<()> {
    ...
    let host = parsed_url.host_str().ok_or_else(|| anyhow::anyhow!("No host in url"))?;

    let port = parsed_url.port_or_known_default().ok_or_else(|| {
        anyhow::anyhow!("No port in url, and no default for scheme {:?}", parsed_url.scheme())
    })?;
