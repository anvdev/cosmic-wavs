use super::{build_service_config, deploy_service, ServiceConfig};
...
pub async fn start_all_local(fork_rpc_url: Option<&str>) -> Result<()> {
    ...
    if deploy_env == "LOCAL" {
        ...
        let TRIGGER_ORIGIN = env::var("TRIGGER_ORIGIN").unwrap_or_default();
        if TRIGGER_ORIGIN == "COSMOS" {}
        // If Cosmos trigger is enabled, deploy service after containers start
        setup_cosmos_environment().await?;
        // conifgure genesis,config,app,keys for node
        let output = Command::new("sh")
            .args(["../cosmos/configure-cosmos.sh"])
            .output()
            .context("Failed to Configure cosmos scripts")?;
        if !output.status.success() {
            return Err(anyhow::anyhow!(
                "Cosmos configure failed: {}",
                String::from_utf8_lossy(&output.stderr)
            ));
        }
        if TRIGGER_ORIGIN == "COSMOS" {}
...
async fn wait_for_rpc(rpc_url: &str) -> Result<()> {
    for _ in 0..60 {
        // 30 seconds timeout
        if let Ok(output) =
            Command::new("cast").args(["block-number", "--rpc-url", rpc_url]).output()
        {
            if output.status.success() {
                return Ok(());
            }
...
async fn wait_for_cosmos_node() -> Result<()> {
    ...
    for _ in 0..60 {
        // 5 minutes timeout
        if let Ok(output) = Command::new("curl").args(["-s", health_url]).output() {
            if output.status.success() {
                return Ok(());
            }
        }
...
fn validate_cosmos_deployment(component_filename: &str, _cosmos_rpc_url: &str) -> Result<()> {
    ...
    if !Path::new(&format!("./compiled/{}", component_filename)).exists() {
        return Err(anyhow::anyhow!("Component file not found: {}", component_filename));
    }
...
async fn deploy_cosmos_contracts() -> Result<()> {
    ...
    if !output.status.success() {
        return Err(anyhow::anyhow!(
            "Cosmos contract deployment failed: {}",
            String::from_utf8_lossy(&output.stderr)
        ));
    }
...
fn setup_cosmos_environment_vars(
    cosmos_rpc_url: &str,
    cosmos_chain_id: &str,
    trigger_event: &str,
) -> Result<()> {
...
async fn build_cosmos_service_config() -> Result<String> {
    ...
    let config =
        ServiceConfig::from_env().context("Failed to load service config from environment")?;

    let config_file =
        build_service_config(config).context("Failed to build service configuration")?;
...
async fn setup_local_cosmos_node() -> Result<()> {
    ...
    let trigger_event = env::var("TRIGGER_EVENT").unwrap_or_else(|_| "cw-infusion".to_string());
...
fn load_cosmos_env_template() -> Result<()> {
    ...
    let template_content =
        fs::read_to_string(template_path).context("Failed to read cosmos template file")?;
...
fn validate_cosmos_environment() -> Result<()> {
    ...
    if deploy_status == "LOCAL" {
        ...
        println!(
            "✓ Skipping local directories for {} deployment (using cw-orch client)",
            deploy_status
        );
    }
